...
Q_LOGGING_CATEGORY(lcAc, "sleepy.services.ac", QtInfoMsg)
Q_LOGGING_CATEGORY(lcAcWorker, "sleepy.services.ac.worker", QtInfoMsg)
...
PipeWireWorker::PipeWireWorker(std::stop_token token, AudioCollector* collector)
    ...
    m_stream = pw_stream_new_simple(pw_main_loop_get_loop(m_loop), "sleepy-shell", props, &events, this);
    if (!m_stream) {
