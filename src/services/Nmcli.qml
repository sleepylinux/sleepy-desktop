...
Singleton {
    ...
    readonly property bool available: deviceStatus !== null
    readonly property bool busy: activeProcesses.length > 0 || rescanProc.running
    ...
    function isSafeInterfaceName(name: string): bool {
    ...
    function executeCommand(args: list<string>, callback: var, stdinPayload: string): void {
        const proc = commandProc.createObject(root);
        proc.cmdArgs = ["nmcli", ...args];
        proc.callback = callback;
        proc.stdinPayload = stdinPayload || "";
    ...
    function executeSecretCommand(args: list<string>, secret: string, callback: var): void {
    ...
    function connectWireless(ssid: string, password: string, bssid: string, callback: var, retryCount: int): void {
        ...
        if (password && password.length > 0)
            executeSecretCommand(cmd, password, done);
        else
            executeCommand(cmd, done);
    }

    function createConnectionWithPassword(ssid: string, bssidUpper: string, password: string, callback: var): void {
        const cmd = [root.nmcliCommandDevice, root.nmcliCommandWifi, "connect", ssid];
        if (bssidUpper && bssidUpper.length > 0)
            cmd.push("bssid", bssidUpper);
        executeSecretCommand(cmd, password, callback);
    }
    ...
    function addHiddenNetwork(ssid: string, password: string, security: string, hidden: bool, callback: var): void {
        ...
        const cmd = [root.nmcliCommandDevice, root.nmcliCommandWifi, "connect", ssid, "hidden", hidden ? "yes" : "no"];
        if (isSecure)
            executeSecretCommand(cmd, password, callback);
        else
    ...
    function getEthernetSpeed(interfaceName: string): void {
        if (!root.isSafeInterfaceName(interfaceName)) {
            root.lastError = qsTr("Invalid network interface name");
            root.ethernetSpeed = "";
            return;
        }
    ...
    function getEthernetDataUsage(interfaceName: string, callback: var): void {
        ...
        dataUsageProc.command = ["cat", `/sys/class/net/${interfaceName}/statistics/rx_bytes`, `/sys/class/net/${interfaceName}/statistics/tx_bytes`];
    ...
    LoggingCategory {
        ...
        name: "sleepy.qml.services.nmcli"
