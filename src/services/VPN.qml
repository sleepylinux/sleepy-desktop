...
// Modified for Sleepy: product identity and shell-free interface telemetry.
...
Singleton {
    ...
    readonly property bool busy: root.connecting || root.disconnecting || statusProc.running
    ...
    function refreshStats(): void {
        ...
        if (root.isSafeInterfaceName(iface)) {
            statsProc.command = ["cat", `/sys/class/net/${iface}/statistics/rx_bytes`, `/sys/class/net/${iface}/statistics/tx_bytes`];
            ...
            if (!pingProc.running) {
                pingProc.command = ["ping", "-c1", "-W2", "-I", iface, "1.1.1.1"];
                pingProc.running = true;
            }
        } else if (iface.length > 0) {
            root.lastError = qsTr("Invalid VPN interface name");
        }
