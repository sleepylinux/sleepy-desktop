...
FocusScope {
    ...
    Keys.onTabPressed: event => {
        if (!actionButton.activeFocus && actionButton.enabled) {
            actionButton.forceActiveFocus();
            event.accepted = true;
        } else if (root.nextTabTarget) {
            root.nextTabTarget.forceActiveFocus();
            event.accepted = true;
        } else {
    ...
    Keys.onBacktabPressed: event => {
        if (actionButton.activeFocus) {
            ...
            event.accepted = true;
        } else if (root.previousTabTarget) {
            root.previousTabTarget.focusLastControl();
            event.accepted = true;
        } else {
            event.accepted = false;
        }
