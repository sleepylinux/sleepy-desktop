...
for qml_file in sorted(source_root.rglob("*.qml")):
    ...
    tokens = [(match.group(), match.start()) for match in token_pattern.finditer(mask_comments_and_strings(source))]
    inline_components = []
    for index in range(len(tokens) - 4):
        ...
        for candidate in range(open_index, len(tokens)):
            ...
            if token == "{":
                ...
                depth == 1
                and token == "id"
                and candidate + 2 < len(tokens)
                and tokens[candidate + 1][0] == ":"
            ):
    ...
    for index in range(len(tokens) - 2):
        if index in inline_token_indexes:
            continue
        if tokens[index][0] == "id" and tokens[index + 1][0] == ":":
