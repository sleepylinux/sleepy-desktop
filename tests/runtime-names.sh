...
check_file() {
  ...
  case "$relative" in
    NOTICE|UPSTREAM.json|src/LICENSE|scripts/import-upstream.sh|tests/runtime-names.sh|tests/upstream-provenance.sh|tests/fixtures/upstream-v2.4.0-parity-inventory.json|tests/lib/parity-validator.sh|tests/parity-validator-test.sh)
      return 0
      ;;
  esac
